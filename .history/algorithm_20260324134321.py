...
Step 2 — run experiment:
  ...
  python finance_conformal.py --npz sp500_20240102_20240229.npz --test_sector Healthcare
...
------------
  ...
  - with_shift=True:  likelihood-ratio weights computed from train vs test-sector
                      ...
                      A logistic classifier is trained to distinguish train-sector
                      from test-sector featurized prefixes; its predicted
                      probabilities give the density ratio weight for each cal
                      series: w_i = P(test | feat_i) / P(train | feat_i).

FEATURE OVERRIDE
----------------
  algorithm.py's _featurize_prefixes uses only the last closing price as the
  single classifier feature.  For log-return / normalized-price data this gives
  near-zero separation between sectors.  This file monkey-patches a richer
  featurizer onto the predictor instance after construction:

    features per series = [mean, std, min, max, last, cumsum_last, AR(1) coef]

  computed over the Y prefix (close prices / returns).  These give the logistic
  classifier a much better chance of distinguishing Technology from other sectors.
=============================================================================
"""
import argparse
import json
...
# algorithm.py uses only the last price (1 feature), which gives the logistic
# classifier near-zero separation on normalized return data.
# This version computes 7 summary statistics over the full Y prefix so the
# classifier has a real chance of separating sectors.
# =============================================================================
def _richer_featurize_prefixes(self, prefixes):
    """
    prefixes: (n, t+1, 1)
    Returns (n, 7) feature matrix:
      [mean, std, min, max, last value, cumulative sum at last step, AR(1) slope]
    All computed over the raw Y series (close price or return, whatever Y holds).
    """
    Y = prefixes[..., 0]          # (n, t+1)
    n, T = Y.shape

    feat_mean = Y.mean(axis=1)
    feat_std  = Y.std(axis=1) + 1e-8
    feat_min  = Y.min(axis=1)
    feat_max  = Y.max(axis=1)
    feat_last = Y[:, -1]
    feat_csum = Y.cumsum(axis=1)[:, -1]

    # AR(1) slope per series: regress y_{1..T} on y_{0..T-1}
    if T >= 2:
        x_ar = Y[:, :-1]   # (n, T-1)
        y_ar = Y[:, 1:]    # (n, T-1)
        ...
        feat_ar1 = np.clip(num / den, -5.0, 5.0)
    else:
        feat_ar1 = np.zeros(n)

    return np.column_stack([
        feat_mean, feat_std, feat_min, feat_max,
        feat_last, feat_csum, feat_ar1,
    ])
...
def _print_weight_diagnostic(weights, t, label=""):
    ...
    Helps diagnose whether the logistic classifier is finding a real signal:
      - uniform=True / std~0  → classifier not separating train vs test sector
      - high std, wide min/max range → meaningful reweighting happening
    Also prints a 5-bucket histogram so you can see the weight distribution shape.
    """
...
def _print_feature_diagnostic(predictor, cal_feat, t):
    """
    Diagnostic 1: compare per-feature mean/std between train and test (ctx)
    prefixes fed to the logistic classifier, and for the cal set.
    |sep| = |mean_train - mean_test| / (std_train + std_test) — a normalised
    separation score.  Near 0 means the classifier has no signal on that feature.
    """
    ...
    feat_names = ["mean", "std", "min", "max", "last", "cumsum", "ar1"]
...
def _print_classifier_diagnostic(predictor, cal_feat, t):
    """
    Diagnostic 2: report the fitted logistic classifier's training accuracy
    and coefficient norm.
      train_acc ≈ 0.5 or coef_norm ≈ 0  →  classifier found no separating direction
      prob1 range narrow around 0.5      →  all density ratio weights will be uniform
    """
    clf = predictor._clf
    if clf is None:
...
def run_finance_experiment(result, test_sector, cal_frac=0.5, alpha=0.1, seed=42,
    ...
    # ── Monkey-patch richer featurizer onto this predictor instance ───────────
    # algorithm.py's default _featurize_prefixes uses only the last closing
    # price (1 feature), which gives near-zero separation on normalized return
    # data.  We bind the richer version (7 summary stats over the full Y prefix)
    # to this instance only, so algorithm.py itself is left unchanged.
    predictor._featurize_prefixes = types.MethodType(
        _richer_featurize_prefixes, predictor
    )
    print(f"  Featurizer      : richer (mean/std/min/max/last/cumsum/ar1 over Y prefix)")
    print()
    ...
    for t in range(T):
        ...
        # cal_scores[i*(t+2) + s] = |y_true - y_pred| for cal series i, step s
        cal_scores = []
        ...
        # Default: uniform weights (overwritten below when with_shift=True)
        ...
        if with_shift and t >= 1:
            ...
            for pred_idx, ctx_idx in [(half1, half2), (half2, half1)]:
                ...
                # Step 1: train LR classifier using ctx_idx half as test-sector reference
                ...
                # Step 2: compute per-series LR weights for cal data.
                # Must be inside the loop — _train_feat_t/_test_feat_t change each swap.
                ...
                # ── Diagnostic 1: feature separation (swap 0 only) ───────────
                # Shows per-feature mean/std for train vs test-ctx vs cal.
                # |sep| near 0 on all features → classifier has no signal.
                if pred_idx is half1 and (t == 1 or t % 10 == 0 or t == T - 1):
                    _print_feature_diagnostic(predictor, cal_feat, t)

                # ── Diagnostic 2: classifier quality (swap 0 only) ───────────
                # train_acc ≈ 0.5 or coef_norm ≈ 0 → classifier found nothing.
                # prob1 range narrow around 0.5 → all weights will be uniform.
                if pred_idx is half1 and (t == 1 or t % 10 == 0 or t == T - 1):
                ...
                assert len(tiled_w) == len(cal_scores_arr), (
                    f"tiled_w length {len(tiled_w)} != cal_scores length {len(cal_scores_arr)}"
                )
            ...
            # Unweighted path: with_shift=False, or t==0 (no prefix history yet)
            alpha_used = alpha_t.copy()
            alpha_next = alpha_t.copy()
            covered_t  = []
            width_t    = []
