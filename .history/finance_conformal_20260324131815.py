...
def _print_weight_diagnostic(weights, t, label=""):
    ...
    if hi > lo:
        ...
        hist_str = "  ".join(
            f"[{edges[i]:.3f},{edges[i+1]:.3f}): {counts[i]}" for i in range(5)
        )
    else:
...
def run_finance_experiment(result, test_sector, cal_frac=0.5, alpha=0.1, seed=42,
    ...
    T = L - 1  # number of prediction steps
    ...
    for t in range(T):
        ...
        #
        # with_shift=True, t>=1:
        #   Cross-split LR weighting. The test set is split into two halves.
        #   For each swap, the *other* half acts as the weighting context so
        #   no test point is used both to estimate weights and to be evaluated.
        #
        #   The logistic classifier in update_weighting_context() is trained to
        #   distinguish featurized train-sector prefixes (label 0) from
        #   featurized test-sector prefixes (label 1).  Each cal series then
        #   gets weight  w_i = P(test | feat_i) / P(train | feat_i)  so that
        #   the conformal quantile reflects the test sector's distribution.
        #
        # with_shift=False, or t==0 (no prefix history yet):
        #   Uniform weights — plain conformal, no sector correction.
        ...
        if with_shift and t >= 1:
            ...
            # Snapshot alpha before predictions; both halves write into alpha_next.
            ...
            for pred_idx, ctx_idx in [(half1, half2), (half2, half1)]:
                ...
                # Step 1: train LR classifier using ctx_idx half as the
                # test-sector reference.  This populates _train_feat_t and
                # _test_feat_t inside the predictor.
                ...
                # Step 2: compute per-series LR weights for cal data against
                # THIS swap's classifier.  Must be inside the loop because
                # _train_feat_t / _test_feat_t are overwritten each iteration.
                # BUG 2 FIX: was computed once outside the loop, so swap=1
                # silently used swap=0's classifier.
                ...
                # Sanity-check shapes before tiling.
                # BUG 3 GUARD: catches shape mismatches loudly rather than
                # silently producing wrong-length weight vectors.
                n_steps = t + 2  # number of cal scores per series
                ...
                # BUG 1 FIX: _scores must be an ndarray, not the raw Python list
                # cal_scores.  _weighted_quantile does numpy operations on it;
                # passing a list caused weights to be silently ignored.
                predictor._scores  = cal_scores_arr   # ndarray, not cal_scores list
                predictor._weights = tiled_w
                predictor._q       = None

                # Diagnostic: print weight summary at key time steps.
                if t == 1 or t % 10 == 0 or t == T - 1:
                    label = "half1-ctx" if list(pred_idx) == list(half1) else "half2-ctx"
                    _print_weight_diagnostic(per_series_w, t, label=label)

                # Predict for this half using the weights from the OTHER half's
                # classifier — the cross-split guarantee.
                for i in pred_idx:
                    x_t    = X_test[i, t+1, :]
                    ...
                    err = 0 if covered else 1
                    # BUG 4 FIX: write ACI update into alpha_next (indexed by i),
                    # not a discarded local.  Both halves accumulate into the same
                    # alpha_next before it becomes alpha_t.
                    alpha_next[i] = alpha_used[i] + gamma_opt * (alpha - err)

            alpha_t = np.clip(alpha_next, 1e-6, 1.0 - 1e-6)

        else:
            # Unweighted path: with_shift=False, or t==0 (no prefix history).
            # predictor._scores / _weights already set to uniform above.
            alpha_used = alpha_t.copy()
            alpha_next = alpha_t.copy()
            covered_t  = []
            width_t    = []
