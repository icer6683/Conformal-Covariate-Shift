    ...
    for t in range(T):
        ...
        if with_shift and t >= 1:
            ...
            for pred_idx, ctx_idx in [(half1, half2), (half2, half1)]:
                ...
                # Step 2: compute per-cal-series LR weights directly.
                # We cannot use calibrate() because it produces one weight per cal
                # series, but cal_scores has n_cal*(t+2) entries (all steps).
                # Solution: featurize each cal series at time t, compute one LR weight
                # per series via the logistic classifier, then tile across (t+2)
                # timesteps so weights and cal_scores align.
                cal_feat = predictor._featurize_prefixes(Y_cal[:, :t+1, :])  # (n_cal, 1)
                per_series_w = predictor._compute_density_ratio_weights(
                    trainX=predictor._train_feat_t,
                    testX=predictor._test_feat_t,
                    evalX=cal_feat,
                )  # (n_cal,)
                tiled_w = np.repeat(per_series_w, t + 2)  # (n_cal*(t+2),)
                predictor._scores  = cal_scores
