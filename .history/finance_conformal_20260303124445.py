#!/usr/bin/env python3
"""
Finance Conformal Prediction using AdaptedCAFHT on real S&P 500 data.
Linear model and cal scores are refitted at each time step t using only
the prefix up to t, mirroring fit_ar_model(train_Y[:, :t+2, :]) in test_conformal.py.
"""
import argparse
import json
...
def run_finance_experiment(result, test_sector, cal_frac=0.5, alpha=0.1, seed=42, gamma_grid=None):
    ...
    for t in range(L):
        ...
        # Refit linear model on prefix up to t — mirrors fit_ar_model(train_Y[:, :t+2, :])
        linear_model.fit(Y_train[:, :t+1, :], X_train[:, :t+1, :],
                         verbose=(t == L - 1))  # only print coefficients on final step
        predictor.noise_std = linear_model.noise_std

        # Recompute cal scores on prefix up to t — mirrors calibrate(cal_Y[:, :t+2, :])
        cal_scores = []
        ...
        # Gamma selection every 20 steps — mirrors test_conformal.py
        if t > 0 and (t % 20 == 0):
        ...
        if (t + 1) % 10 == 0 or t == L - 1:
            print(f"  [t={t+1:3d}/{L}]  coverage={np.mean(covered_t):.3f}  "
