...
USAGE:
  python test_basic_conformal.py
  python test_basic_conformal.py --alpha 0.05 --n_test 200
=============================================================================
"""
...
def test_coverage_with_adapted_cafht(generator, n_train=500, n_test=100, alpha=0.1, 
    ...
    """
    Test Adapted CAFHT algorithm on data with NO covariate shift.
...
def main():
    ...
    print(f"  Rolling window: {args.window_size}")
    ...
    # Test if coverage is valid (within ±5%)
    is_valid = abs(final_coverage - target_coverage) < 0.05
    print(f"Coverage validity: {'✓ PASS' if is_valid else '✗ FAIL'}")
    
    if is_valid:
        print(f"\n🎉 SUCCESS: Basic conformal prediction achieves target coverage!")
        print(f"The algorithm correctly provides ~{target_coverage:.0%} coverage on data with no shift.")
        print(f"Ready to test with covariate shift correction.")
    else:
        print(f"\n❌ FAILURE: Basic conformal prediction failed!")
        print(f"Coverage is {final_coverage:.1%}, should be ~{target_coverage:.1%}")
        print(f"Need to debug the algorithm implementation.")
    ...
    print(f"\n📊 This validates your Adapted CAFHT algorithm from algorithm.py")
    print(f"   Next step: Test with covariate shift using likelihood ratio weighting")
