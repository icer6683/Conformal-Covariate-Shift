...
def _print_weight_diagnostic(weights, t, label=""):
    ...
    w_sum = w.sum()
    w_norm = w / w_sum if w_sum > 0 else w  # normalised for histogram

    is_uniform = bool(np.allclose(w, w[0], rtol=1e-4, atol=1e-6))

    # 5-bucket histogram over raw weights
    lo, hi = w.min(), w.max()
    if hi > lo:
        edges = np.linspace(lo, hi, 6)
        counts, _ = np.histogram(w, bins=edges)
        hist_str = "  ".join(f"[{edges[i]:.3f},{edges[i+1]:.3f}): {counts[i]}" for i in range(5))
    else:
    ...
    for t in range(T):
        ...
        if with_shift and t >= 1:
            ...
            for swap, (pred_idx, ctx_idx) in enumerate([(half1, half2), (half2, half1)]):
                ...
                # Compute LR weights directly (see previous fix for why not calibrate())
                ...
                tiled_w = np.repeat(per_series_w, t + 2)
                predictor._scores  = cal_scores
                predictor._weights = tiled_w
                predictor._q       = None

                # Print weight diagnostic at t=1, every 10 steps, and final step
                if swap == 0 and (t == 1 or t % 10 == 0 or t == T - 1):
                    _print_weight_diagnostic(per_series_w, t, label="half1-ctx")
