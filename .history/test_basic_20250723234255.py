...
"""
=============================================================================
TEST BASIC CONFORMAL COVERAGE - Online Prediction with Covariate Shift
=============================================================================

PURPOSE: Generate data from ts_generator.py and test basic conformal prediction
         with online coverage visualization to demonstrate covariate shift failure.

This creates moving average coverage plots showing how coverage degrades
over time when covariate shift occurs during online prediction.
...
def main():
    ...
    parser.add_argument('--shift_time', type=int, default=250, help='When covariate shift occurs (absolute time)')
    ...
    print(f"  Test timesteps: {args.n_series}")
    ...
    coverage_no_shift_mean = np.mean(results_no_shift['coverage_history'])
    print(f"\nResults (No Shift):")
    ...
    coverage_with_shift_mean = np.mean(results_with_shift['coverage_history'])
    print(f"\nResults (With Shift):")
    print(f"  Target coverage: {target_coverage:.1%}")
    ...
    width_increase = np.mean(results_with_shift['interval_widths']) - np.mean(results_with_shift['interval_widths'])
    ...
    print(f"")
    print(f"Interval width without shift: {np.mean(results_no_shift['interval_widths']):.4f}")
    print(f"Interval width with shift: {np.mean(results_with_shift['interval_widths']):.4f}")
