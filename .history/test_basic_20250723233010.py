...
def run_coverage_experiment(generator, predictor, n_series=500, ar_coef=0.7, noise_std=0.2, 
    ...
    for i in range(n_series):
        ...
        if with_shift:
            ...
            # Debug: Print first few series to check shift is actually applied
            if i < 3:
                print(f"  Debug series {i}:")
                print(f"    Original last value: {original_series[-1, 0]:.3f}")
                print(f"    Shifted last value: {test_series[-1, 0]:.3f}")
                print(f"    Shift difference: {test_series[-1, 0] - original_series[-1, 0]:.3f}")
        else:
