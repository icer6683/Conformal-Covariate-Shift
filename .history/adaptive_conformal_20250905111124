...
USAGE:
  from online_conformal import OnlineConformalPredictor
...
class OnlineConformalPredictor:
    ...
    def predict_with_interval(self, series, update_after=False, true_value=None):
        """
        ...
        # Update conformity scores if requested
        if update_after and true_value is not None:
            new_score = abs(true_value - prediction)
