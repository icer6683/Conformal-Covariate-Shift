...
def plot_time_based_results(results_by_time, target_coverage, covariate_mode="static", with_shift=False):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(20, 5))  # Also adjusted figsize for better horizontal layout
    ...
    # Plot 1: Interval width by time step  
    axes[0].plot(time_steps, interval_widths, 'go-', linewidth=2, markersize=6)
    axes[0].set_xlabel('Time Step t')
    axes[0].set_ylabel('Average Interval Width')
    axes[0].set_title('Prediction Interval Width vs. Time Step')
    axes[0].grid(True, alpha=0.3)
    
    # Plot 2: Moving average of coverage rate
    ...
    if window_size > 1:
        ...
        axes[1].plot(time_steps, coverage_rates, 'b-', alpha=0.5, linewidth=1, label='Raw Coverage')
        axes[1].plot(moving_time_steps, moving_avg, 'r-', linewidth=3, 
                    label=f'Moving Avg (window={window_size})')
        axes[1].axhline(y=target_coverage, color='red', linestyle='--', alpha=0.7,
                       label=f'Target ({target_coverage:.1%})')
    else:
        axes[1].plot(time_steps, coverage_rates, 'b-', linewidth=2, label='Coverage Rate')
        axes[1].axhline(y=target_coverage, color='red', linestyle='--',
                       label=f'Target ({target_coverage:.1%})')
    
    axes[1].set_xlabel('Time Step t')
    axes[1].set_ylabel('Coverage Rate')
    axes[1].set_title('Coverage Rate Trend Over Time')
    axes[1].legend()
