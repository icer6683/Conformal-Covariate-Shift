...
def run_finance_experiment(result, test_sector, cal_frac=0.5, alpha=0.1, seed=42, gamma_grid=None, with_shift=False):
    ...
    T = L - 1  # number of prediction steps (we predict step t+1 for t in range(T))
    ...
    for t in range(T):
        ...
        for i in range(n_cal):
            for s in range(t + 2):  # steps 0..t+1
                ...
                cal_scores.append(abs(y_true - y_pred))
        cal_scores         = np.array(cal_scores, dtype=float)
        predictor._scores  = cal_scores
        predictor._weights = np.ones(len(cal_scores), dtype=float)
        predictor._q       = None
        ...
        # ── prediction loop ────────────────────────────────────────────────────
        # Mirrors test_conformal.py exactly:
        #   with_shift + t>=1: cross-split update_weighting_context -> calibrate -> predict
        #   otherwise:         calibrate (uniform weights)           -> predict
        #
        # KEY FIX: calibrate() must be called AFTER update_weighting_context() so
        # that _compute_density_ratio_weights() runs and populates _weights with
        # likelihood-ratio values.  Setting _weights manually before the context
        # update (as before) always left them uniform.
        if with_shift and t >= 1:
            ...
            # Cross-split: half1 predicted using half2 as weighting context, then swap.
            # Mirrors test_conformal.py lines:
            #   predictor.update_weighting_context(..., test_prefixes=test_data[idx_half2,...])
            #   predictor.calibrate(cal_Y[:, :t+2, :])
            #   for i in idx_half1: predict...
            for pred_idx, ctx_idx in [(half1, half2), (half2, half1)]:
                # Step 1: set the weighting context (stores train/test features for LR)
                predictor.update_weighting_context(
                    train_prefixes=train_prefixes,
                    test_prefixes=Y_test[ctx_idx, :t+1, :],
                    is_shifted=True,
                )
                # Step 2: recompute calibration weights using the new context.
                # calibrate() internally calls _compute_density_ratio_weights on cal
                # prefixes vs the stored train/test features — this is what actually
                # fills _weights with non-uniform likelihood-ratio values.
                # We use a Y_cal_like array shaped (n_cal, t+2, 1) so calibrate()
                # sees the same prefix length it expects.
                predictor.calibrate(Y_cal[:, :t+2, :])
                # Overwrite _scores with our linear-model residuals (calibrate() uses
                # the internal AR model; we need scores from our linear model instead).
                predictor._scores = cal_scores

                if t == 1 and list(pred_idx) == list(half1):
