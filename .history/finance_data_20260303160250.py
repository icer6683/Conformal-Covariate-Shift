...
------
  ...
  meta      : list[dict]            sector, industry, exchange, shares_outstanding per ticker
  cov_names : list[str]             covariate column names

COVARIATE LAG LOGIC
-------------------
  OvernightGapReturn     NOT lagged  (Open_t - Close_{t-1}) / Close_{t-1}, known at open
  Above52wLowReturn      NOT lagged  (Open_t - fixed_52w_low) / fixed_52w_low
                                     fixed_52w_low = min(Low) over the 252 trading days
                                     strictly BEFORE the sample start date
  TurnoverRatio_lag1     lagged 1    Volume_{t-1} / (shares_outstanding * Close_{t-1})
  DailyRangeReturn_lag1  lagged 1    (High - Low)_{t-1} / Close_{t-2}
...
def load_series(
    ...
    """
    # Fetch 252 trading days of Low prices before the sample window for 52w low
    low_52w_map = _fetch_52w_lows(tickers, start, interval)
    ...
    # Fetch metadata first so shares_outstanding is available for covariates
    meta_lookup = {m["ticker"]: m for m in _fetch_metadata(tickers)}
    ...
    for ticker in tickers:
        ...
        y = (close / np.where(open_ != 0, open_, np.nan) - 1).reshape(-1, 1)
        y = np.nan_to_num(y, nan=0.0)

        shares  = meta_lookup.get(ticker, {}).get("shares_outstanding", None)
        low_52w = low_52w_map.get(ticker, None)
        cov_df  = _build_covariates(df, extra=extra_covariates,
                                    shares_outstanding=shares, low_52w=low_52w)
        x      = cov_df.values
    ...
    meta      = [meta_lookup[t] for t in valid_tickers]
...
def _fetch_52w_lows(tickers: list[str], start: str, interval: str = "1d") -> dict[str, float]:
    """
    For each ticker, download up to 252 trading days of daily Low prices that end
    strictly before `start`, and return the minimum Low as a fixed scalar.
    ...
    start_str = start_pre.strftime("%Y-%m-%d")
    end_str   = end_pre.strftime("%Y-%m-%d")

    print(f"[INFO] Fetching pre-window 52w lows  {start_str} → {end_str} ...")
    result = {}
    for ticker in tickers:
        try:
            df = yf.download(
            ...
            result[ticker] = low_val if np.isfinite(low_val) and low_val > 0 else None
        except Exception as e:
            print(f"[WARNING] Could not fetch pre-window data for {ticker}: {e}")
            result[ticker] = None
    n_ok = sum(v is not None for v in result.values())
    print(f"[INFO] 52w lows fetched for {n_ok}/{len(tickers)} tickers.")
    return result


def _build_covariates(df: pd.DataFrame, extra: bool,
                      shares_outstanding: Optional[float] = None,
                      low_52w: Optional[float] = None) -> pd.DataFrame:
    """
    Build scale-invariant covariates comparable across tickers of any price level.

    Covariates
    ----------
    OvernightGapReturn     : (Open_t - Close_{t-1}) / Close_{t-1}   [not lagged]
    Above52wLowReturn      : (Open_t - fixed_52w_low) / fixed_52w_low
                             fixed_52w_low = min(Low) over 252 trading days before
                             the sample window (fetched separately at pull time)  [not lagged]
    TurnoverRatio_lag1     : Volume_{t-1} / (shares_outstanding * Close_{t-1})
                             if shares_outstanding unavailable, falls back to
                             z-scored volume within the series                    [lagged 1]
    DailyRangeReturn_lag1  : (High - Low)_{t-1} / Close_{t-2}  volatility proxy  [lagged 1]
    """
    cov   = pd.DataFrame(index=df.index)
    close = df["Close"]
    prev  = close.shift(1)   # Close_{t-1}
    prev2 = close.shift(2)   # Close_{t-2}

    # ── Not lagged (known at market open before today's close) ────────────────
    cov["OvernightGapReturn"] = (df["Open"] - prev) / prev

    # Fixed 52-week low: scalar computed from data before the sample window.
    # Falls back to the in-sample series min if not provided.
    _low = low_52w if (low_52w is not None and low_52w > 0) else float(df["Low"].min())
    cov["Above52wLowReturn"] = (df["Open"] - _low) / _low

    # ── Lagged 1 day ──────────────────────────────────────────────────────────
    vol_lag1 = df["Volume"].shift(1)
    if shares_outstanding is not None and shares_outstanding > 0:
        # Turnover ratio: fraction of shares traded yesterday
        cov["TurnoverRatio_lag1"] = vol_lag1 / (shares_outstanding * prev)
    else:
        # Fallback: z-score volume within this series
        vol_mean = vol_lag1.mean()
        vol_std  = vol_lag1.std(ddof=1)
        cov["TurnoverRatio_lag1"] = (
            (vol_lag1 - vol_mean) / vol_std if vol_std > 0 else 0.0
        )

    if extra:
        daily_range = (df["High"] - df["Low"]).shift(1)
        cov["DailyRangeReturn_lag1"] = daily_range / prev2
...
def _fetch_metadata(tickers: list[str]) -> list[dict]:
    ...
    for ticker in tickers:
        ...
        try:
            ...
            entry["shares_outstanding"] = info.get("sharesOutstanding",  None)
        except Exception as e:
