...
- **Extra step — EXPERIMENTAL revised whole-trajectory algorithm (2026-06-12; commit `f826993`, all confined to `--revised` paths + one new file, trivially revertible).** Replaces the single `X_1` classifier with a **per-step classifier `p̂_t` on the growing covariate prefix `X_{1:t}`**, giving per-step weights `Ŵ_{i,t}` and a per-step correction `η_{j,t}` that inflates only step t (same `ε_i`, predictor, D_ACI, γ, ACI). New file `core/weighted_cafht_whole_revised.py` (`WeightedCAFHTWholeTrajectoryRevised`, a subclass overriding only `predict_bands`; imports helpers — relaxing §0 self-containment since it is experimental). Added a `--revised` flag + a `featurize_prefix` (running {mean,std,last} of the observed prefix, **including the target prefix `Y_{0:h}`** — never `Y_{h+1}`) to the synthetic / medical / finance whole runners and the synthetic/medical multi-seed wrappers. Separately changed the evaluation metric in all three whole runners' `_coverage_metrics`: joint coverage now **omits the first `ceil(H/10)` steps** (skips the ACI cold-start; `joint_coverage_full` kept for reference). **Results (revised vs original `full`, omit metric):** medical 0.971→**0.947** (toward target) with width **1833→1501 (−18%)**; synthetic dynamic 0.739→**0.781** and *narrower* (2.33→2.18) — both wins; synthetic static neutral (~0.98, over-covers either way); finance no help — Tech 0.799→0.791, Util 0.933→0.915, both with **~23–37 δ_∞ bands/window**. Probe showed the δ_∞ flood is NOT perfect separation (per-step `train_acc` only ~0.7–0.8) but **overconfidence × 39-step multiplicity**: the 15-feature classifier saturates a few outliers to `p̂≈1` (raw `ŵ≈10⁶` via `prob_clip=1e-6`), and a series counts as δ_∞ if *any* of its 39 per-step classifiers blows up. Fix (not yet applied): regularize the per-step classifier (lower `C` / looser `prob_clip` / fewer features). Net: the per-step revision helps where the shift evolves and is learnable from a short prefix without over-separating (medical, synthetic-dynamic), neutral on synthetic-static, unhelpful on finance. **Files touched:** NEW `core/weighted_cafht_whole_revised.py`; MODIFIED (added `--revised` + `featurize_prefix` + omit-metric) `synthetic/synthetic_runner_whole.py`, `medical/medical_runner_whole.py`, `finance/finance_runner_whole.py`; MODIFIED (thread `--revised`) `synthetic/multi_seed_synthetic_whole.py`, `medical/multi_seed_medical_whole.py`. The omit-metric edit also lives in those three whole runners' `_coverage_metrics`. (Finance has no multi-seed wrapper. The deleted `REVISED_WHOLE_TRAJECTORY.md` scratch-plan is gone.)
...
### 6. Medical runner pair — `medical_runner_whole.py` and `medical_runner_last.py`  *(DONE — 2026-05-30; commit `11b4fab`; done before 7 per user)*
...
### 8. Synthetic runner pair — `synthetic_runner_whole.py` and `synthetic_runner_last.py`  *(DONE — 2026-05-30; commit `89b4142`; done before 6/7 per user)*
